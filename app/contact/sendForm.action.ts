...
export default async function sendFormAction({name, country, city, email, subject, message}: sendFormAction, token: string | null) {
    ...
    const emailVerified =  /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i.test(email)
