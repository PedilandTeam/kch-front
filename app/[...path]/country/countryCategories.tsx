...
async function getMostUsedCategories(countryCode: string) {
  ...
  try {
    result = await (await API_ROUTES.CATEGOREIS.MOST_USED(countryCode, 120)).json()
    return result
  } catch (e) {
    console.log(e);
...
export const CountryCategories = async ({currentCountry}: CountryCategoriesProps) => {
  ...
  return (
    <div className="mod-categories my-8 sm:my-16">
      ...
      <CountryCategoriesItems currentCountry={currentCountry} units={units} recentlyUpdatedCategories={mostUsedCategories} />
    </div>
  );
};
