...
export default function CategoryFilterItem({category}: CategoryFilterItemProps) {
  ...
  return (
    <div className="item flex justify-between items-center">
      {/**  @ts-expect-error */}
      <input onChange={inputClickHandler} id={`category-select-${category.name}`} value={category.id} type="checkbox" checked={Array.isArray(parsedSearchParams.category) ? !!parsedSearchParams.category.find(param => param == category.id) : parsedSearchParams.category == category.id} className="checkbox checkbox-sm" />
      <label htmlFor={`category-select-${category.name}`} className="text-sm">{category.name}</label>
    </div>
  )
