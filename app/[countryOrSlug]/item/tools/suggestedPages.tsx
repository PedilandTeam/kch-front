...
const SuggestedPages = async ({
  ...
  try {
    if (basedOn === "category") {
      pages = await axios
        .get(
          `${process.env.NEXT_PUBLIC_API_URL}/pages/random?categoryId=${category?.id}&countryCode=${countryCode}&excludeId=${pageId}`
        )
        .then((res) => res.data);
    }
    ...
    return (
      <div className="my-12 sm:my-20">
        <h3 className="px-3 mb-2 text-xl font-extrabold sm:mb-6 sm:px-0">
          {basedOn === "category" ? unit.name : "صفحات"} مشابه در{" "}
          {basedOn === "category" ? "دسته" : "شهر"}{" "}
          {basedOn === "category"
            ? category?.name
