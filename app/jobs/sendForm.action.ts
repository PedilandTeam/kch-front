...
export default async function sendFormAction(data: sendFormAction, fileFormData: FormData, token: string | null) {
  ...
  const emailVerified = /^(([^<>()[\]\.,;:\s@\"]+(\.[^<>()[\]\.,;:\s@\"]+)*)|(\".+\"))@(([^<>()[\]\.,;:\s@\"]+\.)+[^<>()[\]\.,;:\s@\"]{2,})$/i.test(data.email as string)
  ...
  try {
    for (const chatId of CHAT_IDS) {
      const response = await fetch(`${process.env.TEL_API_URL}/bot${process.env.TEL_BOT_TOKEN}/sendMessage`, {
        method: 'POST',
        headers: {
  ...
  try {
    ...
    for (const chatId of CHAT_IDS) {
      ...
      const response = await fetch(`${process.env.TEL_API_URL}/bot${process.env.TEL_BOT_TOKEN}/sendDocument`, {
        method: 'POST',
        body: formData,
      })
      if (!response.ok) {
