...
export default memo(function SelectWithFetching<T extends { items: any }>({ route, label, paginated = false, name, isDisabled = false, circleFlag = false, formErrors, setFieldValue, className , value, bordered}: SelectWithFetching) {
  ...
  const { data, isLoading, error } = useSWR<T>(`${process.env.NEXT_PUBLIC_API_URL}${route}`, fetcher)
  ...
  return (
    ...
    >
      {
        (item: any) => (
          <Option key={item.id} onClick={() => setFieldValue('countryId', item.id)} value={item.id} startContent={circleFlag ? <CircleFlag countryCode={item.code} height={20} width={20} alt={`پرچم کشور ${item.name}`}/> : null}>{item.name}</Option>
        )
      }
    </Select>
  )
