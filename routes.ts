...
const baseFetch = async (
  { path, method = 'GET', params, body, headers }: requestType,
  { cache, revalidate }: configType
): Promise<Response> => {
  return new Promise((resolve, reject) => {
    ...
    let url = queryParametrs
      ? `${API_URL}/${path}?${queryParametrs}`
      : `${API_URL}/${path}`;
...
export const API_ROUTES = {
  PAGES: {
    ...
    GET_ONE: (id: string, revalidate?: number, cache?: requestCacheType) => {
      return baseFetch(
        { path: `pages/${id}`, method: 'GET', headers: { 'api-key': API_KEY } },
        { ...(cache && { cache }), ...(revalidate && { revalidate }) }
      );
    },
  },
      ...
      return baseFetch(
        {
          path: 'countries',
          method: 'GET',
          params: { ...(isBetweenZeroAndOne(status) && { status }) },
        },
        { ...(cache && { cache }), ...(revalidate && { revalidate }) }
      );
    },
  ...
  CITIES: {
      ...
      return baseFetch(
        {
          path: 'cities',
          method: 'GET',
          params: { page, limit, ...(countryCode && { countryCode }) },
        },
        { ...(cache && { cache }), ...(revalidate && { revalidate }) }
      );
    },
    BY_COUNTRY: (
      countryCode: string,
      filter: CityFiltersNamespace.byCountry
    ) => {
      return baseFetch(
        {
          ...
          params: {
            ...
            ...(filter.unitIds && { unitIds: filter.unitIds }),
            ...(filter.categoryIds && { categoryIds: filter.categoryIds }),
          },
        },
        { revalidate: 10 }
      );
  ...
  CATEGOREIS: {
    ...
    RECENTLY_UPDATED: (revalidate?: number, cache?: requestCacheType) => {
      return baseFetch(
        { path: 'categories/recentlyUpdated' },
        { ...(cache && { cache }), ...(revalidate && { revalidate }) }
      );
    },
    MOST_USED: (
      ...
      return baseFetch(
        {
          path: 'categories/mostUsed',
          params: { limit, ...(countryCode && { countryCode }) },
        },
        { ...(cache && { cache }), ...(revalidate && { revalidate }) }
      );
    },
    BY_COUNTRY: (
      countryCode: string,
      filter: CategoryFiltersNamespace.byCountry
    ) => {
      return baseFetch(
        {
          path: `categories/country/${countryCode}`,
          params: {
            page: filter.page,
            limit: filter.limit,
            ...(filter.cityIds && { cityIds: filter.cityIds }),
            ...(filter.slug && { slug: filter.slug }),
            ...(filter.unitIds && { unitIds: filter.unitIds }),
          },
        },
