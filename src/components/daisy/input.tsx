...
export default function Input({
    ...
    return (
        <div className={`form-control w-full max-w-full ${className}`}>
            <div className={`input relative ${className} ${bordered ? 'input-bordered' : ''} ${isInvalid ? 'input-error' : ''}`}>
                {startContent && (
            ...
            <label className="label">
                ...
                <span className="label-text-alt text-error min-h-[1 ch]">{isInvalid ? errorMessage : ''}</span>
            </label>
        </div>
    );
}