import Image from "next/image";

import phImage from "@/assets/images/ph-terms.webp";

export const TermsPageHead = () => {
  return (
    <div className="_terms-page-head relative">
      <Image
        className="aspect-3/2 rounded-xl object-cover brightness-[.60]"
        src={phImage}
        placeholder="blur"
        width={390}
        height={390}
        alt="a tennis ball in a court"
        priority
      />
      <div className="absolute top-1/2 left-1/2 w-full -translate-x-1/2 -translate-y-1/2 px-4">
        <div className="wrap space-y-2 text-center text-white">
          <h1 className="text-3xl font-bold">قوانین و مقررات</h1>
          <h2 className="text-xl font-medium">طرفدار حقوق‌تون هستیم!</h2>
        </div>
      </div>
    </div>
  );
};
